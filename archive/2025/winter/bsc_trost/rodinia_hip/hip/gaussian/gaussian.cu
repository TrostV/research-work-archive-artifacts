      ...
      if (argv[i][0]=='-') {// flag
        flag = argv[i][1];
	      ...
	      printf("Create matrix internally in parse, size = %d \n", Size);
				hipHostMalloc((void**)&a, sizeof(float)*Size*Size, hipHostMallocCoherent|hipHostMallocMapped);
	      create_matrix(a, Size);
				
				hipHostMalloc((void**)&b, sizeof(float)*Size, hipHostMallocCoherent|hipHostMallocMapped);
	      //b = (float *) malloc(Size * sizeof(float));
	      for (j =0; j< Size; j++)
	    	b[j]=1.0;
				
				hipHostMalloc((void**)&m, sizeof(float)*Size*Size, hipHostMallocCoherent|hipHostMallocMapped);
...
void InitProblemOnce(char *filename)
{
	...
	hipHostMalloc((void**)&a, sizeof(float)*Size*Size, hipHostMallocCoherent);
	//a = (float *) malloc(Size * Size * sizeof(float));
	 
	InitMat(a, Size, Size);
	//printf("The input matrix a is:\n");
	//PrintMat(a, Size, Size);
	hipHostMalloc((void**)&b, sizeof(float)*Size, hipHostMallocCoherent);
	//b = (float *) malloc(Size * sizeof(float));
	
	InitAry(b, Size);
	//printf("The input array b is:\n");
	//PrintAry(b, Size);
	hipHostMalloc((void**)&m, sizeof(float)*Size*Size, hipHostMallocCoherent);
	//m = (float *) malloc(Size * Size * sizeof(float));
}
